...
export default function EditArticlePage() {
    ...
    const [formData, setFormData] = useState<{
        ...
        title: '',
        slug: '', // Will be set from params initially, but might be editable?
        excerpt: '',
        metaTitle: '',
        metaDescription: '',
        keywords: '',
    ...
    useEffect(() => {
        if (slug) {
            const loadArticle = () => {
                try {
                    const item = localStorage.getItem(`page-${slug}`);
                    if (item) {
                        const parsed = JSON.parse(item);
                        setFormData({
                            ...parsed,
                            // If some fields are missing in old data, default them
                            island: parsed.island || 'Gran Canaria',
                            category: parsed.category || 'Hotels',
                            featuredMediaType: parsed.featuredMediaType || 'url',
    ...
    const handleTitleChange = (title: string) => {
        // Only auto-update slug if we are creating new? Or if user wants it?
        // In edit mode, usually we DON'T auto-update slug to prevent breaking links, unless explicit.
        // For now, I will let user update it manually if they want, but auto-update metaTitle.
        setFormData({
            ...formData,
            title,
            metaTitle: title,
    ...
    const handleSave = (status: 'Concept' | 'Gepubliceerd') => {
        ...
        // If slug changed, we should probably delete the old key?
        // But keeping it simple for now: save to new key.

        // Check if slug changed from the original param
        if (formData.slug !== slug) {
            // If slug changes, we might want to delete the valid "page-slug" key, OR just create a new one.
            // Let's create new and advise user.
            // Actually, for a proper "Move", we should delete old.
            if (confirm('U heeft de slug veranderd. Er wordt een nieuwe pagina aangemaakt en de oude blijft bestaan (of moet handmatig verwijderd worden). Doorgaan?')) {
                // proceed
            } else {
                return;
            }
        ...
        try {
            ...
            // If slug changed, we could delete old one: localStorage.removeItem(`page-${slug}`);

            window.dispatchEvent(new Event('content-updated'));

            console.log('Saving article:', articleData);
            alert(`Content bijgewerkt als ${status}!`);
            router.push('/admin/content');
        } catch (error) {
    ...
    if (loading) {
        return <div className="p-12 text-center text-gray-500">Artikel laden...</div>;
    }
        ...
        <div>
            ...
            <div className="mb-8 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">Content Bewerken</h1>
                        <p className="text-gray-600 mt-1">Bewerk artikel: {formData.title}</p>
                    </div>
                </div>
                <div className="flex gap-3">
                    <button
                        onClick={() => handleSave('Concept')}
                        className={`flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-900 font-semibold px-6 py-3 rounded-lg transition-colors ${formData.status === 'Concept' ? 'ring-2 ring-primary' : ''}`}
                    >
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 bg-primary hover:bg-primary-dark text-white font-semibold px-6 py-3 rounded-lg transition-colors ${formData.status === 'Gepubliceerd' ? 'ring-2 ring-offset-2 ring-primary' : ''}`}
                    >
